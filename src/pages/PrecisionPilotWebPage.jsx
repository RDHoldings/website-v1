...
export function PrecisionPilotWebPage({ variant }) {
  ...
  useEffect(() => {
    document.title = isTest
      ? 'Precision Pilot (pre-release test) | Red Domino Holdings'
      : 'Precision Pilot | Red Domino Holdings'
  }, [isTest])
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#0a0a0a] text-[#e5e7eb]">
      ...
      {isTest && iframeSrc ? (
        <div className="border-b border-white/10 bg-black/40 px-4 py-2 font-mono text-xs text-[#9ca3af] md:px-6">
      ...
      <main className="flex min-h-0 flex-1 flex-col">
        {iframeSrc ? (
          <iframe
            ...
            src={iframeSrc}
            className="min-h-[50vh] w-full min-h-0 flex-1 border-0 bg-black"
      ...
      <footer className="border-t border-white/10 px-6 py-6 text-center text-xs text-[#6b7280]">
